...
public class Core
{
    ...
    public void TakeScreenshot() {
        ...
        final File file = new File(String.valueOf(GameWindow.getGameModeCacheDir()) + File.separator + Core.GameSaveWorld + File.separator + "thumb.png");
        ...
        Texture.forgetTexture(String.valueOf(GameWindow.getGameModeCacheDir()) + File.separator + Core.GameSaveWorld + File.separator + "thumb.png");
    }
    ...
    public void TakeFullScreenshot(String filename) {
        try {
            ...
            final File file = new File(String.valueOf(GameWindow.getScreenshotDir()) + File.separator + filename);
            for (int i = 0; i < pixels.length; ++i) {
                final int bindex = i * 3;
                pixels[i] = (0xFF000000 | (buffer.get(bindex) & 0xFF) << 16 | (buffer.get(bindex + 1) & 0xFF) << 8 | (buffer.get(bindex + 2) & 0xFF) << 0);
            }
    ...
    public void EndFrameText(final int nPlayer) {
        ...
        final TextureFBO current = this.OffscreenBuffer.Current;
    ...
    public boolean loadOptions() throws IOException {
        ...
        final File inFile = new File(String.valueOf(GameWindow.getCacheDir()) + File.separator + "options.bin");
        final File newFile = new File(String.valueOf(GameWindow.getCacheDir()) + File.separator + "options.ini");
            ...
            try {
                ...
                while ((line = br.readLine()) != null) {
                    if (line.startsWith("version=")) {
                        ...
                        final String name = line.replaceFirst("controller=", "").trim();
                        if (name.isEmpty()) {
                            continue;
                        }
                ...
                if (!this.doneNewSaveFolder) {
                    ...
                    try {
                        for (final String path : gameModes) {
                            previousSave = new File(String.valueOf(GameWindow.getCacheDir()) + File.separator + path);
                            newSave = new File(String.valueOf(GameWindow.getSaveDir()) + File.separator + path);
    ...
    private void searchFolders(final File file, final String relative) {
        if (file.isDirectory()) {
            final File newFile = new File(String.valueOf(this.saveFolder) + File.separator + relative);
            newFile.mkdir();
            final String[] internalNames = file.list();
            for (int i = 0; i < internalNames.length; ++i) {
                this.searchFolders(new File(String.valueOf(file.getAbsolutePath()) + File.separator + internalNames[i]), String.valueOf(relative) + File.separator + internalNames[i]);
            }
        }
        else {
            ...
            try {
                ...
                for (int playerIndex = 0; playerIndex < 4; ++playerIndex) {
                    if (Core.bAutoZoom[playerIndex]) {
                        if (!autoZoom.isEmpty()) {
                            autoZoom = String.valueOf(autoZoom) + ",";
                        }
                        autoZoom = String.valueOf(autoZoom) + (playerIndex + 1);
                    }
                }
    ...
    public static void restartApplication(final Runnable runBeforeRestart) throws IOException {
        try {
            final String java = String.valueOf(System.getProperty("java.home")) + "/bin/java";
            ...
            final StringBuffer cmd = new StringBuffer(String.valueOf(java) + " " + (Object)vmArgsOneLine);
    ...
    public KahluaTable getScreenModes() {
        ...
        final File newFile = new File(String.valueOf(LuaManager.getLuaCacheDir()) + File.separator + "screenresolution.ini");
        int c = 1;
        try {
            if (!newFile.exists()) {
                newFile.createNewFile();
    ...
    public void updateKeyboard() {
        if (Core.CurrentTextEntryBox == null) {
            ...
            while (Keyboard.next()) {
            	if (!Keyboard.getEventKeyState()) {// XXX AddedCodesInHere: Ctrl Key
                ...
                if (Keyboard.getEventKeyState()) {
                    if (Keyboard.getEventKey() == 28) {
                                ...
                                if (b.Lines.size() < b.getMaxLines()) {
                                    if (b.TextEntryCursorPos != b.ToSelectionIndex) {
                                        ...
                                        if (b.internalText.length() > 0) {
                                            b.internalText = String.valueOf(b.internalText.substring(0, l)) + "\n" + b.internalText.substring(h);
                                        }
                                        else {
                                            b.internalText = "\n";
                                        }
                                        ...
                                        final String text = String.valueOf(b.internalText.substring(0, textOffset)) + "\n" + b.internalText.substring(textOffset);
                                        b.SetText(text);
                        ...
                        if (Keyboard.getEventKey() == 15 || Keyboard.getEventKey() == 58) {
                            continue;
                        }
                        if (Keyboard.getEventKey() == 199) {
                            ...
                            if (!Core.CurrentTextEntryBox.Lines.isEmpty()) {
                                Core.CurrentTextEntryBox.TextEntryCursorPos = Core.CurrentTextEntryBox.TextOffsetOfLineStart.get(Core.CurrentTextEntryBox.CursorLine) + ((String)Core.CurrentTextEntryBox.Lines.get(Core.CurrentTextEntryBox.CursorLine)).length();
                            }
                            if (!Keyboard.isKeyDown(42) && !Keyboard.isKeyDown(54)) {
                                Core.CurrentTextEntryBox.ToSelectionIndex = Core.CurrentTextEntryBox.TextEntryCursorPos;
                            }
                            ...
                            if (Core.CurrentTextEntryBox.CursorLine > 0) {
                                ...
                                if (column > ((String)Core.CurrentTextEntryBox.Lines.get(Core.CurrentTextEntryBox.CursorLine)).length()) {
                                    column = ((String)Core.CurrentTextEntryBox.Lines.get(Core.CurrentTextEntryBox.CursorLine)).length();
                                }
                            ...
                            if (Keyboard.getEventKey() == 29 || Keyboard.getEventKey() == 157 || Keyboard.getEventKey() == 56 || Keyboard.getEventKey() == 184) {
                                continue;
                            }
                            if (Keyboard.getEventKey() == 203) {
                                ...
                                if ((Keyboard.getEventKey() == 211 || Keyboard.getEventKey() == 14) && Core.CurrentTextEntryBox.TextEntryCursorPos != Core.CurrentTextEntryBox.ToSelectionIndex) {
                                    ...
                                    Core.CurrentTextEntryBox.internalText = String.valueOf(Core.CurrentTextEntryBox.internalText.substring(0, i)) + Core.CurrentTextEntryBox.internalText.substring(h2);
                                    ...
                                    if (Core.CurrentTextEntryBox.TextEntryCursorPos > 0) {
                                        Core.CurrentTextEntryBox.internalText = String.valueOf(Core.CurrentTextEntryBox.internalText.substring(0, Core.CurrentTextEntryBox.TextEntryCursorPos)) + Core.CurrentTextEntryBox.internalText.substring(Core.CurrentTextEntryBox.TextEntryCursorPos + 1);
                                    }
                                    else {
...
//                                        Core.CurrentTextEntryBox.internalText = String.valueOf(Core.CurrentTextEntryBox.internalText.substring(0, textOffset2 - 1)) + Core.CurrentTextEntryBox.internalText.substring(textOffset2);
//                                    }
//                                    if (Core.CurrentTextEntryBox.TextEntryCursorPos > 0) {
...
//                                    Core.CurrentTextEntryBox.onTextChange();
                                    ...
                                    if (Core.CurrentTextEntryBox.TextEntryCursorPos != Core.CurrentTextEntryBox.ToSelectionIndex) {
                                        ...
                                        Core.CurrentTextEntryBox.internalText = String.valueOf(Core.CurrentTextEntryBox.internalText.substring(0, j)) + clip + Core.CurrentTextEntryBox.internalText.substring(h3);
                                        Core.CurrentTextEntryBox.ToSelectionIndex = j + clip.length();
                                        Core.CurrentTextEntryBox.TextEntryCursorPos = j + clip.length();
                                    }
                                    else {
                                        if (Core.CurrentTextEntryBox.TextEntryCursorPos < Core.CurrentTextEntryBox.internalText.length()) {
                                            Core.CurrentTextEntryBox.internalText = String.valueOf(Core.CurrentTextEntryBox.internalText.substring(0, Core.CurrentTextEntryBox.TextEntryCursorPos)) + clip + Core.CurrentTextEntryBox.internalText.substring(Core.CurrentTextEntryBox.TextEntryCursorPos);
                                        }
                                        else {
                                            Core.CurrentTextEntryBox.internalText = String.valueOf(Core.CurrentTextEntryBox.internalText) + clip;
                                        }
                                    ...
                                    if (Core.CurrentTextEntryBox.TextEntryCursorPos != Core.CurrentTextEntryBox.ToSelectionIndex) {
                                        ...
                                        if (Core.CurrentTextEntryBox.internalText.length() > 0) {
                                            Core.CurrentTextEntryBox.internalText = String.valueOf(Core.CurrentTextEntryBox.internalText.substring(0, i)) + Keyboard.getEventCharacter() + Core.CurrentTextEntryBox.internalText.substring(h2);
                                        }
                                        else {
                                            Core.CurrentTextEntryBox.internalText = new StringBuilder().append(Keyboard.getEventCharacter()).toString();
                                        }
                                        ...
                                        if (textOffset2 < Core.CurrentTextEntryBox.internalText.length()) {
                                            Core.CurrentTextEntryBox.internalText = String.valueOf(Core.CurrentTextEntryBox.internalText.substring(0, textOffset2)) + Keyboard.getEventCharacter() + Core.CurrentTextEntryBox.internalText.substring(textOffset2);
                                        }
                                        else {
                                            final UITextBox2 currentTextEntryBox8 = Core.CurrentTextEntryBox;
//                                            currentTextEntryBox8.internalText = String.valueOf(currentTextEntryBox8.internalText) + Keyboard.getEventCharacter();
    ...
    public void init(final int width, final int height) throws LWJGLException {
        ...
        if (Core.bIsSteam) {
            GameWindow.OSValidator.isWindows();
        }
        Display.setVSyncEnabled(Core.OptionVSync);
        GL11.glEnable(3553);
    ...
    public void setScreenSize(final int width, final int height) {
        if (Core.width != width || height != Core.height) {
            ...
            try {
                LuaEventManager.triggerEvent("OnResolutionChange", (Object)oldWidth, (Object)oldHeight, (Object)width, (Object)height);
            }
            catch (Exception ex) {
                ex.printStackTrace();
            }
    ...
    public void setSaveFolder(final String newSaveFolder) {
        if (!this.saveFolder.equals(newSaveFolder)) {
            ...
            newDir = new File(String.valueOf(newSaveFolder) + File.separator + "mods");
            if (!newDir.exists()) {
                newDir.mkdir();
            }
            final String to = String.valueOf(this.saveFolder) + File.separator;
            this.saveFolder = newSaveFolder;
            this.copyPasteFolders(String.valueOf(to) + "mods");
    ...
    public void deleteDirectoryRecusrively(final String directory) {
        ...
        for (int i = 0; i < internalNames.length; ++i) {
            final File fileDelete = new File(String.valueOf(directory) + File.separator + internalNames[i]);
            if (fileDelete.isDirectory()) {
                this.deleteDirectoryRecusrively(String.valueOf(directory) + File.separator + internalNames[i]);
            }
            else {
                fileDelete.delete();
            }
    ...
    public ArrayList<Integer> getDefaultZoomLevels() {
        return (ArrayList<Integer>)this.OffscreenBuffer.getDefaultZoomLevels();
    }
    ...
    public void ResetLua(final boolean sp, final String reason) {
        ...
        LuaEventManager.triggerEvent("OnResetLua", (Object)reason);
    }
    ...
    public ColorInfo getMpTextColor() {
        if (this.mpTextColor == null) {
            this.mpTextColor = new ColorInfo((float)(Rand.Next(135) + 120), (float)(Rand.Next(135) + 120), (float)(Rand.Next(135) + 120), 255.0f);
        }
